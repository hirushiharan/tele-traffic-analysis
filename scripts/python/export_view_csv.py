"""
Program: MySQL Data Export to CSV
...
to a CSV file. The script utilizes environment variables for database credentials, ensuring security and flexibility.
...
def export_dataframe_to_csv(df, file_path):
    """
    Export the provided Pandas DataFrame to a CSV file.
...
def main():
    """
    Main function to perform data export from a MySQL view to a CSV file.
    ...
    try:
        ...
        csv_file_path = 'documents/call_details_view.csv'

        # Export DataFrame to a CSV file
        export_dataframe_to_csv(df, csv_file_path)

        log(f"Data exported successfully to {csv_file_path}", "INFO")
